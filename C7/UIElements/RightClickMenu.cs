...
public partial class RightClickTileMenu : RightClickMenu {
	...
	private bool isUnitFortified(MapUnit unit, Dictionary<EntityId, bool> uiStates) {
	...
	public void ResetItems(Tile tile, Dictionary<EntityId, bool> uiUpdatedUnitStates = null) {
		RemoveAll();
	...
	public void SelectUnit(EntityId id) {
		using (var gameDataAccess = new UIGameDataAccess()) {
			...
			if (toSelect != null && toSelect.owner == game.controller) {
				...
				ResetItems(toSelect.location, new Dictionary<EntityId, bool>() { { toSelect.id, false } });
			}
		}
	...
	public void ForAll(int tileX, int tileY, bool isFortify) {
		using (var gameDataAccess = new UIGameDataAccess()) {
			...
			Dictionary<EntityId, bool> modified = new Dictionary<EntityId, bool>();
