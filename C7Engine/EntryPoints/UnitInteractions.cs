...
namespace C7Engine
{
	...
	public class UnitInteractions
	{
		...
		public static void waitUnit(GameData gameData, EntityID id)
		{
			foreach (MapUnit unit in gameData.mapUnits)
			{
				if (unit.id == id)
				{
					log.Verbose("Found matching unit with guid " + id + " of type " + unit.GetType().Name + "; adding it to the wait queue");
					waitQueue.Enqueue(unit);
				}
			}
			log.Warning("Failed to find a matching unit with guid " + id);
		}
	}
}
