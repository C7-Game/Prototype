...
namespace BuildDevSave {
	class Program {
		...
		static void Main(string[] args) {
			if (args.Length < 2) {
				Console.Out.WriteLine("provide civ3 SAV relative path and output filename as command line arguments");
				return;
			}
			...
			C7SaveFormat.Save(output, Path.Join(C7DefaultSaveDir, outputFilename));
		}
	}
}
